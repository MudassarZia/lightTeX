...
protected:
  ...
  std::vector<CompletionItem> makeItems(const std::vector<QString> &labels) {
...
TEST_F(CompletionWidgetTest, ItemChosenCarriesCommandTriggerKind) {
  ...
  if (spy.count() == 1) {
    ...
    EXPECT_TRUE(tk == TriggerKind::Command || tk == TriggerKind::Argument);
  }
}
...
TEST_F(CompletionWidgetTest, FromJsonPrefersTextEditNewText) {
  ...
  obj["textEdit"] = QJsonObject{
      ...
      {"range",
       QJsonObject{{"start", QJsonObject{{"line", 0}, {"character", 1}}},
