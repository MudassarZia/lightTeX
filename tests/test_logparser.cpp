...
TEST(LogParser, ParseWarning) {
    std::string log = "LaTeX Warning: Reference `fig:1' on page 1 undefined on input line 15.\n";
    auto messages = parseLog(log);
    ASSERT_EQ(messages.size(), 1u);
...
TEST(LogParser, ParsePackageWarning) {
    std::string log = "Package hyperref Warning: Token not allowed in a PDF string on input line 20.\n";
    auto messages = parseLog(log);
    ASSERT_EQ(messages.size(), 1u);
    EXPECT_EQ(messages[0].kind, MessageKind::Warning);
    ASSERT_TRUE(messages[0].line.has_value());
...
TEST(LogParser, ParseBadBox) {
    std::string log = "Overfull \\hbox (15.0pt too wide) in paragraph at lines 20--22\n";
    auto messages = parseLog(log);
    ASSERT_EQ(messages.size(), 1u);
...
TEST(LogParser, ParseSuccessLog) {
    ...
    if (log.empty()) GTEST_SKIP() << "Fixture not found";
    auto messages = parseLog(log);
...
TEST(LogParser, ParseMultipleErrors) {
    ...
    int errorCount = 0;
    for (const auto& msg : messages) {
        if (msg.kind == MessageKind::Error) ++errorCount;
    }
