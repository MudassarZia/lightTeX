...
SyntaxHighlighterBridge::SyntaxHighlighterBridge(QTextDocument *parent)
  ...
  connect(&reparseTimer_, &QTimer::timeout, this,
          &SyntaxHighlighterBridge::doReparse);
}

void SyntaxHighlighterBridge::setTheme(const lighttex::theme::Theme &theme) {
  colorMap_[TokenKind::Command] =
      QColor(QString::fromStdString(theme.syntax.command));
  colorMap_[TokenKind::Environment] =
      QColor(QString::fromStdString(theme.syntax.environment));
  colorMap_[TokenKind::MathDelimiter] =
      QColor(QString::fromStdString(theme.syntax.math));
  colorMap_[TokenKind::MathContent] =
      QColor(QString::fromStdString(theme.syntax.math));
  colorMap_[TokenKind::Comment] =
      QColor(QString::fromStdString(theme.syntax.comment));
  colorMap_[TokenKind::Text] =
      QColor(QString::fromStdString(theme.colors.foreground));
  colorMap_[TokenKind::Bracket] =
      QColor(QString::fromStdString(theme.syntax.bracket));
  colorMap_[TokenKind::Number] =
      QColor(QString::fromStdString(theme.syntax.number));
  colorMap_[TokenKind::String] =
      QColor(QString::fromStdString(theme.syntax.string));
  colorMap_[TokenKind::Error] =
      QColor(QString::fromStdString(theme.syntax.error));

  // Re-parse with new colors
  scheduleReparse();
}

void SyntaxHighlighterBridge::scheduleReparse() { reparseTimer_.start(); }
...
void SyntaxHighlighterBridge::highlightBlock(const QString &text) {
  if (text.isEmpty() || !isHighlighting_)
    return;
  ...
  auto it =
      std::lower_bound(cachedEvents_.begin(), cachedEvents_.end(), probe,
