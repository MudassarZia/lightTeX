...
std::optional<TokenKind> Highlighter::classifyNode(const char *nodeType,
  ...
  // Brackets
  if (std::strcmp(nodeType, "{") == 0 || std::strcmp(nodeType, "}") == 0 ||
  ...
  // Operators
  if (std::strcmp(nodeType, "&") == 0 || std::strcmp(nodeType, "=") == 0 ||
      std::strcmp(nodeType, "_") == 0 || std::strcmp(nodeType, "^") == 0) {
  ...
  if ((std::strcmp(nodeType, "text") == 0 ||
       std::strcmp(nodeType, "word") == 0) &&
      !hasChildren) {
    return TokenKind::Text;
  }

  // Errors — only leaf ERROR nodes (no children)
  // Large ERROR subtrees from partial parses should still classify children
  // normally
...
std::vector<HighlightEvent>
Highlighter::walkTree(TSTree *tree, const std::string & /*source*/) {
