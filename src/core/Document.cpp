...
std::string Document::text() const { return buffer_.text(); }

size_t Document::length() const { return buffer_.length(); }

size_t Document::lineCount() const { return buffer_.lineCount(); }

bool Document::isEmpty() const { return buffer_.empty(); }

std::string Document::line(size_t idx) const { return buffer_.line(idx); }
...
BufferDelta Document::replace(size_t start, size_t end,
...
BufferDelta Document::applyTransaction(const Transaction &txn) {
  ...
  for (const auto &edit : txn.edits) {
    size_t adjStart =
        static_cast<size_t>(static_cast<int>(edit.start) + cumulativeShift);
    size_t adjEnd =
        static_cast<size_t>(static_cast<int>(edit.end) + cumulativeShift);
    ...
    int editDelta = static_cast<int>(edit.text.size()) -
                    static_cast<int>(edit.end - edit.start);
