...
void PieceTable::insert(size_t pos, const std::string& text) {
    ...
    if (pieces_.empty()) {
        ...
        if (pieceIdx >= pieces_.size()) {
            ...
            Piece right{existing.buffer, existing.start + offset, existing.length - offset};
            auto it = pieces_.erase(pieces_.begin() + static_cast<ptrdiff_t>(pieceIdx));
    ...
    for (const auto& p : pieces_) {
        ...
        if (pEnd <= start || offset >= end) {
            ...
            if (pEnd > end) {
                ...
                newPieces.push_back({p.buffer, p.start + rightOffset, p.length - rightOffset});
            }
        }
        offset += p.length;
    }
...
size_t PieceTable::lineCount() const {
    if (lineStartsDirty_) rebuildLineStarts();
    return lineStarts_.size();
}

std::string PieceTable::line(size_t lineIdx) const {
    if (lineStartsDirty_) rebuildLineStarts();
    if (lineIdx >= lineStarts_.size()) return "";
...
std::pair<size_t, size_t> PieceTable::charToLineCol(size_t charIdx) const {
    ...
    size_t total = length();
    if (charIdx > total) charIdx = total;
