...
std::vector<int> BracketMatcher::findMatchingBrackets(const QString &text,
                                                      int cursorPos) const {
  ...
  for (int i = 0; i < bracketCount; ++i) {
    if (ch == openBrackets[i]) {
      int match = findMatchingForward(text, cursorPos, openBrackets[i],
                                      closeBrackets[i]);
      if (match >= 0)
        return {cursorPos, match};
      return {};
    }
  }
  ...
  for (int i = 0; i < bracketCount; ++i) {
    if (ch == closeBrackets[i]) {
      int match = findMatchingBackward(text, cursorPos, openBrackets[i],
                                       closeBrackets[i]);
      if (match >= 0)
        return {match, cursorPos};
      return {};
    }
  }
  ...
  if (cursorPos > 0) {
    ...
    for (int i = 0; i < bracketCount; ++i) {
      if (prev == closeBrackets[i]) {
        int match = findMatchingBackward(text, cursorPos - 1, openBrackets[i],
                                         closeBrackets[i]);
        if (match >= 0)
          return {match, cursorPos - 1};
      }
    }
  }
...
std::vector<int> BracketMatcher::findMatchingBrackets(const QTextDocument *doc,
                                                      int cursorPos) const {
  if (!doc)
    return {};
  ...
  for (int i = 0; i < bracketCount; ++i) {
    if (ch == openBrackets[i]) {
      int match = findMatchingForward(doc, cursorPos, openBrackets[i],
                                      closeBrackets[i]);
      if (match >= 0)
        return {cursorPos, match};
      return {};
  ...
  for (int i = 0; i < bracketCount; ++i) {
    if (ch == closeBrackets[i]) {
      int match = findMatchingBackward(doc, cursorPos, openBrackets[i],
                                       closeBrackets[i]);
      if (match >= 0)
        return {match, cursorPos};
      return {};
  ...
  if (cursorPos > 0) {
    ...
    for (int i = 0; i < bracketCount; ++i) {
      if (prev == closeBrackets[i]) {
        int match = findMatchingBackward(doc, cursorPos - 1, openBrackets[i],
                                         closeBrackets[i]);
        if (match >= 0)
          return {match, cursorPos - 1};
      }
