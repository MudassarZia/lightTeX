...
FindReplaceBar::FindReplaceBar(EditorWidget *editor, QWidget *parent)
  ...
  connect(searchInput_, &QLineEdit::textChanged, this,
          &FindReplaceBar::onSearchChanged);
  connect(prevButton_, &QPushButton::clicked, this,
          &FindReplaceBar::findPrevious);
  connect(nextButton_, &QPushButton::clicked, this, &FindReplaceBar::findNext);
  connect(replaceButton_, &QPushButton::clicked, this,
          &FindReplaceBar::replaceCurrent);
  connect(replaceAllButton_, &QPushButton::clicked, this,
          &FindReplaceBar::replaceAll);
  connect(closeButton_, &QPushButton::clicked, this, &FindReplaceBar::closeBar);
...
void FindReplaceBar::setTheme(const lighttex::theme::Theme &theme) {
  ...
  setStyleSheet(
      QString("FindReplaceBar { background: %1; border-bottom: 1px solid %3; }"
              ...
              "padding: 2px 4px; }"
              "QPushButton { background: transparent; color: %2; border: 1px "
              "solid %3; "
              "padding: 2px 6px; }"
...
void FindReplaceBar::updateHighlights() {
  ...
  for (int i = 0; i < static_cast<int>(matches_.size()); ++i) {
    ...
    sel.format.setBackground((i == currentMatch_) ? currentColor : matchColor);
