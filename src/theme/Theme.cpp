...
Theme loadTheme(const std::string& tomlStr) {
    ...
    if (auto colors = tbl["colors"].as_table()) {
        theme.colors.background = getStr(*colors, "background", theme.colors.background);
        theme.colors.foreground = getStr(*colors, "foreground", theme.colors.foreground);
        theme.colors.cursor = getStr(*colors, "cursor", theme.colors.cursor);
        theme.colors.selection = getStr(*colors, "selection", theme.colors.selection);
        theme.colors.lineHighlight = getStr(*colors, "line_highlight", theme.colors.lineHighlight);
        theme.colors.gutter = getStr(*colors, "gutter", theme.colors.gutter);
        theme.colors.gutterForeground = getStr(*colors, "gutter_foreground", theme.colors.gutterForeground);
    }

    if (auto syntax = tbl["syntax"].as_table()) {
        theme.syntax.command = getStr(*syntax, "command", theme.syntax.command);
        theme.syntax.environment = getStr(*syntax, "environment", theme.syntax.environment);
...
std::string Theme::toStyleSheet() const {
    ...
    qss += "QStatusBar QLabel { background: transparent; color: " +
           ui.statusbarFg + "; }\n";
    ...
    qss += "QToolBar { background: " + ui.panelBg +
           ...
           "; color: " + colors.foreground + "; }\n";
    qss += "QToolButton { background: transparent; color: " +
           colors.foreground + "; padding: 4px 8px; border: none; }\n";
    qss += "QToolButton:hover { background: " + colors.selection + "; }\n";

    // Menus
    qss += "QMenuBar { background: " + ui.panelBg +
           "; color: " + colors.foreground + "; }\n";
    qss += "QMenu { background: " + ui.panelBg +
           "; color: " + colors.foreground +
    ...
    qss += "QScrollArea { background: " + colors.background + "; border: none; }\n";
