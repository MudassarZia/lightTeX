...
void LightTexStatusBar::setCursorPosition(int line, int col) {
  positionLabel_->setText(QString("Ln %1, Col %2").arg(line).arg(col));
}

void LightTexStatusBar::setCompileStatus(
    lighttex::compiler::CompileStatus status) {
  compileStatus_ = status;
  updateStatusLabel();
}

void LightTexStatusBar::setEngine(lighttex::compiler::Engine engine) {
  engineLabel_->setText(QString::fromStdString(
      std::string(lighttex::compiler::engineDisplayName(engine))));
}
