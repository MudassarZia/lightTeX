...
LspClient::LspClient(QObject *parent) : QObject(parent) {
  ...
  connect(&changeTimer_, &QTimer::timeout, this, &LspClient::onDebouncedChange);
}
  ...
  params["capabilities"] = QJsonObject{
      {"textDocument",
       QJsonObject{
           ...
           {"hover", QJsonObject{{"contentFormat", QJsonArray{{"plaintext"}}}}},
...
void LspClient::didOpen(const QString &uri, const QString &text) {
  ...
  if (!initialized_) {
    pendingOps_.push_back([this, uri, text]() { didOpen(uri, text); });
    return;
  }

  QJsonObject params;
  params["textDocument"] = QJsonObject{
      {"uri", uri}, {"languageId", "latex"}, {"version", 1}, {"text", text}};
...
void LspClient::onDebouncedChange() {
  if (!isRunning() || pendingChangeUri_.isEmpty())
    return;

  QJsonObject params;
  params["textDocument"] =
      QJsonObject{{"uri", pendingChangeUri_}, {"version", nextId_}};
  params["contentChanges"] =
      QJsonArray{QJsonObject{{"text", pendingChangeText_}}};
