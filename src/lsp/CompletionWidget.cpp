...
void CompletionWidget::showCompletions(
    ...
    QString prevLabel;
    if (currentRow() >= 0 &&
        currentRow() < static_cast<int>(items_.size())) {
    ...
    if (!wasVisible && cursorPos > triggerPos_ + 1) {
        QTimer::singleShot(0, this, [this]() {
            if (isVisible()) emit refreshRequested();
        });
    }
}
...
void CompletionWidget::positionPopup() {
    if (!editor_ || triggerPos_ < 0) return;
    ...
    if (auto* screen = QApplication::screenAt(globalPos)) {
        ...
        if (globalPos.y() + h > geom.bottom()) {
            globalPos.setY(
                editor_->mapToGlobal(cursorRect.topLeft()).y() - h);
        }
